...
double diff_cost(vector<double> coeff, double duration,
  ...
  为了实现这一点，我们可以使用一些优化技术，例如最小二乘法或梯度下降法, 使得轨迹上的坐标和导数与目标尽可能接近。
...
double collision_circles_cost_spiral(
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      */
      auto circle_center_x = cur_x + CIRCLE_OFFSETS[c] * std::cos(cur_yaw); // <- fix
      auto circle_center_y = cur_y + CIRCLE_OFFSETS[c] * std::sin(cur_yaw); // <- fix
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral, State main_goal) {
  /* 
  计算一条螺旋路径与主目标之间的距离成本
  The last point on the spiral should be used to check how close we are to the Main (center) goal. 
  That way, spirals that end closer to the lane center-line, and that are collision free, will be preferred.
  */
  auto n = spiral.size();

  /*
  TODO-distance between last point on spiral and main goal: 
  How do we calculate the distance between the last point on the spiral (spiral[n-1]) 
  and the main goal (main_goal.location). 
