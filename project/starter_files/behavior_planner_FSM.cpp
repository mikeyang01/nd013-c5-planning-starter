...
double BehaviorPlannerFSM::get_look_ahead_distance(const State & ego_state) {
  ...
  // the distance you will need to come to a stop while traveling at speed V and using a comfortable deceleration.
  ...
  look_ahead_distance =
    std::min(std::max(look_ahead_distance, _lookahead_distance_min), _lookahead_distance_max);
...
State BehaviorPlannerFSM::state_transition(
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    */
    ...
    /* 
    TODO: It turns out that when we teleport, the car is always at speed zero. 
    ...
    DECEL_TO_STOP状态下使用距离而不是速度来判断车辆是否需要停车。让我们来看看这个问题。
    */
